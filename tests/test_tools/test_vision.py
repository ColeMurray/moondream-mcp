...
from moondream_mcp.models import CaptionLength, CaptionResult, QueryResult, DetectionResult, PointingResult
from moondream_mcp.moondream import ModelLoadError, ImageProcessingError
...
class TestVisionTools:
    ...
    def test_register_vision_tools(self, mock_mcp: MagicMock, mock_client: AsyncMock) -> None:
        ...
        registered_functions = [call[1]['func'] if 'func' in call[1] else call[0][0] for call in tool_calls]
        
        # Verify all expected tools are registered
        tool_names = [func.__name__ for func in registered_functions if hasattr(func, '__name__')]
        expected_tools = [
            'caption_image',
            'query_image', 
            'detect_objects',
        ...
        for expected_tool in expected_tools:
            assert any(expected_tool in name for name in tool_names), f"Tool {expected_tool} not found"
    ...
    async def test_caption_image_success(
        ...
        for call in mock_mcp.tool.call_args_list:
            if len(call[0]) > 0 and hasattr(call[0][0], '__name__') and 'caption' in call[0][0].__name__:
    ...
    @pytest.mark.asyncio
    async def test_caption_image_invalid_length(self, mock_mcp: MagicMock, mock_client: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_caption_image_model_error(self, mock_mcp: MagicMock, mock_client: AsyncMock) -> None:
    ...
    async def test_query_image_success(
        ...
        query_func = None
        for call in mock_mcp.tool.call_args_list:
            if len(call[0]) > 0 and hasattr(call[0][0], '__name__') and 'query' in call[0][0].__name__:
                query_func = call[0][0]
                break
    ...
    @pytest.mark.asyncio
    async def test_query_image_empty_inputs(self, mock_mcp: MagicMock, mock_client: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_detect_objects_success(self, mock_mcp: MagicMock, mock_client: AsyncMock) -> None:
        """Test successful object detection."""
        from moondream_mcp.models import DetectedObject, BoundingBox
        ...
        detect_func = None
        for call in mock_mcp.tool.call_args_list:
            if len(call[0]) > 0 and hasattr(call[0][0], '__name__') and 'detect' in call[0][0].__name__:
    ...
    @pytest.mark.asyncio
    async def test_point_objects_success(self, mock_mcp: MagicMock, mock_client: AsyncMock) -> None:
        """Test successful visual pointing."""
        from moondream_mcp.models import PointedObject, Point
        ...
        point_func = None
        for call in mock_mcp.tool.call_args_list:
            if len(call[0]) > 0 and hasattr(call[0][0], '__name__') and 'point' in call[0][0].__name__:
                point_func = call[0][0]
                break
    ...
    async def test_analyze_image_caption_operation(
        ...
        analyze_func = None
        for call in mock_mcp.tool.call_args_list:
            if len(call[0]) > 0 and hasattr(call[0][0], '__name__') and 'analyze' in call[0][0].__name__ and 'batch' not in call[0][0].__name__:
    ...
    @pytest.mark.asyncio
    async def test_analyze_image_invalid_operation(self, mock_mcp: MagicMock, mock_client: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_analyze_image_invalid_json_parameters(self, mock_mcp: MagicMock, mock_client: AsyncMock) -> None:
    ...
    async def test_batch_analyze_images_success(
        ...
        batch_func = None
        for call in mock_mcp.tool.call_args_list:
            if len(call[0]) > 0 and hasattr(call[0][0], '__name__') and 'batch' in call[0][0].__name__:
                batch_func = call[0][0]
                break
    ...
    @pytest.mark.asyncio
    async def test_batch_analyze_images_invalid_paths(self, mock_mcp: MagicMock, mock_client: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_batch_analyze_images_mixed_results(self, mock_mcp: MagicMock, mock_client: AsyncMock) -> None:
