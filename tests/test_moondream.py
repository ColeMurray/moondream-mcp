...
class TestMoondreamClient:
    ...
    def test_preprocess_image(self, client: MoondreamClient, sample_image: Image.Image) -> None:
    ...
    @pytest.mark.asyncio
    async def test_load_image_from_file_not_found(self, client: MoondreamClient) -> None:
    ...
    @pytest.mark.asyncio
    async def test_load_image_from_file_too_large(self, client: MoondreamClient, tmp_path: Path) -> None:
        """Test loading file that's too large."""
        # Create a file that exceeds size limit
        large_file = tmp_path / "large.jpg"
        large_file.write_bytes(b"x" * (client.config.max_file_size_mb * 1024 * 1024 + 1))
    ...
    async def test_load_image_from_file_success(
        self, mock_aiofiles_open: AsyncMock, client: MoondreamClient, sample_image: Image.Image, tmp_path: Path
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_load_image_from_url_http_error(self, client: MoondreamClient) -> None:
        ...
        with patch("aiohttp.ClientSession.get") as mock_get:
            ...
            with pytest.raises(ImageProcessingError, match="Failed to download image: HTTP 404"):
                await client._load_image_from_url("https://example.com/notfound.jpg")

    @pytest.mark.asyncio
    async def test_load_image_from_url_wrong_content_type(self, client: MoondreamClient) -> None:
        ...
        with patch("aiohttp.ClientSession.get") as mock_get:
            ...
            with pytest.raises(ImageProcessingError, match="URL does not point to an image"):
    ...
    @pytest.mark.asyncio
    async def test_caption_image_success(self, client: MoondreamClient, sample_image: Image.Image) -> None:
        """Test successful image captioning."""
        with patch.object(client, "_ensure_model_loaded") as mock_ensure_model, \
             patch.object(client, "_load_image", return_value=sample_image) as mock_load_image, \
            ...
            mock_loop.run_in_executor.return_value.set_result({"caption": "A red square"})
    ...
    async def test_caption_image_model_error(self, client: MoondreamClient) -> None:
        ...
        with patch.object(client, "_ensure_model_loaded", side_effect=ModelLoadError("Model failed to load")):
            
            with pytest.raises(ModelLoadError, match="Model failed to load"):
                await client.caption_image("test.jpg")

    @pytest.mark.asyncio
    async def test_query_image_success(self, client: MoondreamClient, sample_image: Image.Image) -> None:
        ...
        with patch.object(client, "_ensure_model_loaded") as mock_ensure_model, \
            ...
            mock_loop = MagicMock()
            mock_loop.run_in_executor.return_value = asyncio.Future()
            mock_loop.run_in_executor.return_value.set_result({"answer": "Yes, it is red"})
            mock_get_loop.return_value = mock_loop
    ...
    @pytest.mark.asyncio
    async def test_detect_objects_success(self, client: MoondreamClient, sample_image: Image.Image) -> None:
    ...
    @pytest.mark.asyncio
    async def test_point_objects_success(self, client: MoondreamClient, sample_image: Image.Image) -> None:
    ...
    async def test_model_loading_error(self, client: MoondreamClient) -> None:
        ...
        with patch("moondream_mcp.moondream.AutoModelForCausalLM.from_pretrained", side_effect=Exception("Model not found")):
            
            with pytest.raises(ModelLoadError, match="Failed to load Moondream model"):
                await client._load_model()

    @pytest.mark.asyncio
    async def test_semaphore_concurrency_control(self, client: MoondreamClient, sample_image: Image.Image) -> None:
