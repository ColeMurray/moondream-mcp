...
class ImageAnalysisRequest(BaseModel):
    ...
    image_path: str = Field(
        ...,
        description="Path to image file (local path or URL)",
        min_length=1
    )
...
class CaptionRequest(ImageAnalysisRequest):
    ...
    stream: bool = Field(
        default=False,
        description="Whether to stream the caption generation"
    )
...
class BoundingBox(BaseModel):
    ...
    x: float = Field(
        ...,
        description="X coordinate (normalized 0.0 to 1.0)",
        ge=0.0,
        le=1.0
    )
    ...
    y: float = Field(
        ...,
        description="Y coordinate (normalized 0.0 to 1.0)",
        ge=0.0,
        le=1.0
    )
    ...
    width: float = Field(
        ...,
        description="Width (normalized 0.0 to 1.0)",
        ge=0.0,
        le=1.0
    )
    ...
    height: float = Field(
        ...,
        description="Height (normalized 0.0 to 1.0)",
        ge=0.0,
        le=1.0
    )
...
class DetectedObject(BaseModel):
    ...
    name: str = Field(
        ...,
        description="Name of detected object"
    )
...
class DetectionResult(AnalysisResult):
    ...
    object_name: Optional[str] = Field(
        None,
        description="Name of object that was searched for"
    )
    
    total_found: int = Field(
        default=0,
        description="Total number of objects found",
        ge=0
    )
...
class PointedObject(BaseModel):
    ...
    name: str = Field(
        ...,
        description="Name of pointed object"
    )
    
    confidence: float = Field(
        ...,
        description="Pointing confidence (0.0 to 1.0)",
        ge=0.0,
        le=1.0
    )
...
class PointingResult(AnalysisResult):
    ...
    points: List[PointedObject] = Field(
        default_factory=list,
        description="List of pointed objects"
    )
...
class BatchAnalysisResult(BaseModel):
    ...
    total_successful: int = Field(
        ...,
        description="Total number of successful analyses",
        ge=0
    )
